...
export default function Header({ toggleChat, toggleTheme, isDark, openShare }) {
  return (
    <header className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 h-16 flex-shrink-0 z-20 transition-colors">
      <div className="h-full px-4 md:px-6 flex items-center justify-between max-w-7xl mx-auto w-full">
        <Link to="/" className="flex items-center gap-2 group">
          ...
          <span className="font-bold text-gray-900 dark:text-white group-hover:text-orange-600 transition-colors">
            GourmetList
          </span>
        </Link>
        ...
        <div className="flex items-center gap-2">
          {toggleChat && (
            <button 
              onClick={toggleChat} 
              className="lg:hidden btn-icon relative dark:text-gray-300 dark:hover:bg-gray-800"
            >
